...
ターミナルの代わりにデスクトップウィンドウで使いたい場合は、こちらを起動してください（追加インストール不要、標準ライブラリのTkinterのみで動作）。

```bash
python gui.py
```

- 上部の設定バーで言語・ペルソナ・モデル名・LM Studio URL・間隔・タイムアウト・TTSの有効/無効を変更し、**Start** で開始、**Stop** でいつでも停止できます（設定はCLIの `race_engineer.py` と共通）
- 左ペインに現在のテレメトリ（LLMに送信しているものと同じ内容）、右ペインにタイムスタンプ付きのエンジニア無線ログが表示されます
- LMUが未起動・セッション外のときはステータスバーにその旨が表示され、自動的に接続を待ち続けます
